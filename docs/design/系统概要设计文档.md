...
| **技术栈** | Vue3 + FastAPI + MySQL + Tortoise ORM | 技术栈成熟，适合单体架构，Tortoise ORM简化数据库操作 |
...
使用 Tortoise ORM 实现数据模型，简化数据库操作。

#### 4.1.1 用户模型 (`models/user.py`)
...
class Post(models.Model):
    ...
    category = fields.CharEnumField(
        enum_type=EnumType(('life', 'research', 'professional')),
        null=False
    )
...
class PostAttachment(models.Model):
    ...
    type = fields.CharEnumField(
        enum_type=EnumType(('file', 'paper', 'link')),
        null=False
    )
    file_path = fields.CharField(max_length=255, null=True)
    paper = fields.ForeignKeyField('models.Paper', null=True)
...
class Comment(models.Model):
    ...
    content = fields.TextField(null=False)
    parent = fields.ForeignKeyField('models.Comment', null=True, related_name='replies')
    created_at = fields.DatetimeField(auto_now_add=True)
...
#### 4.1.6 下载中心模型 (`models/download.py`)
...
class Download(models.Model):
    ...
    upload_user = fields.ForeignKeyField('models.User', related_name='downloads')
...
class DownloadAttachment(models.Model):
    """下载中心附件模型"""
    id = fields.IntField(pk=True, auto_increment=True)
    download = fields.ForeignKeyField('models.Download', related_name='attachments')
    file_path = fields.CharField(max_length=255, null=False)
    file_name = fields.CharField(max_length=255, null=False)
    created_at = fields.DatetimeField(auto_now_add=True)
    
    class Meta:
        table = "download_attachments"
```

#### 4.1.7 AI对话模型 (`models/ai.py`)
...
class AIMessage(models.Model):
    """AI消息模型"""
    id = fields.IntField(pk=True, auto_increment=True)
    conversation = fields.ForeignKeyField('models.AIConversation', related_name='messages')
    role = fields.CharEnumField(
        enum_type=EnumType(('user', 'assistant')),
        null=False
    )
    content = fields.TextField(null=False)
